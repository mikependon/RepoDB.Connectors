...
namespace RepoDb.Connector.AuroraDb.MySqlConnector.IntegrationTests.Operations
{
    ...
    public class ConnectionTest
    {
        ...
        public void TestAuroraDbConnectionServerVersionAndDatabaseForOpenConnection()
        {
            ...
            Assert.AreEqual("RepoDb.Connector.AuroraDb.MySqlConnector", connection.Database, StringComparer.Ordinal);
        }
        ...
        [TestMethod]
        public void TestAuroraDbConnectionWrappedConnectionUnwrapsToMySqlConnection()
        {
            // Setup
            using var connection = new AuroraDbConnection(Database.ConnectionString);
            connection.Open();
            ...
            Assert.IsTrue(wrapped.IsWrapperFor<MySqlConnection>());
            Assert.AreEqual(ConnectionState.Open, wrapped.Unwrap<MySqlConnection>().State);
        }
        ...
        public void ThrowOnAuroraDbConnectionOpenWithInvalidPassword()
        {
            ...
            // Assert
            var exception = Assert.Throws<MySqlException>(Act);
            Assert.AreEqual(MySqlErrorCode.AccessDenied, exception.ErrorCode);
            Assert.AreEqual(ConnectionState.Closed, connection.State);
        }
        ...
        public void ThrowOnAuroraDbConnectionOpenWithInvalidDatabase()
        {
            ...
            // Assert
            var exception = Assert.Throws<MySqlException>(Act);
            Assert.AreEqual(MySqlErrorCode.UnknownDatabase, exception.ErrorCode);
        }
        ...
        public void ThrowOnAuroraDbConnectionExceptionCarriesSqlState()
        {
            ...
            // Assert - ER_NO_SUCH_TABLE
            var exception = Assert.Throws<AuroraDbException>(Act);
            Assert.AreEqual("42S02", exception.SqlState, StringComparer.Ordinal);
            Assert.AreEqual((int)MySqlErrorCode.NoSuchTable, exception.Number);
            Assert.IsInstanceOfType<MySqlException>(exception.InnerException);
        }
