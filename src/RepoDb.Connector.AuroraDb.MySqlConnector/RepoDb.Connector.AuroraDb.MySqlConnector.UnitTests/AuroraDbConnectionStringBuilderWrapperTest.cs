...
namespace RepoDb.Connector.AuroraDb.MySqlConnector.UnitTests
{
    ...
    public sealed class AuroraDbConnectionStringBuilderWrapperTest
    {
        ...
        {
            // Setup
            var connectionString = "Server=localhost;Database=TestDb;Plugins=failover;";
        ...
        {
            // Setup
            var connectionString = "Server=localhost;Port=3307;Database=TestDb;";

            // Act
            var builder = new AuroraDbConnectionStringBuilder(connectionString);

            // Assert
            Assert.AreEqual(3307U, builder.Port);
        }
        ...
        public void TestAuroraDbConnectionStringBuilderConnectionStringForRoundTrip()
        {
            ...
            var builder = new AuroraDbConnectionStringBuilder
            {
                Server = "my-cluster.cluster-abc.eu-west-1.rds.amazonaws.com",
                Port = 3306,
                Database = "TestDb",
                UserId = "admin",
                Password = "password",
            ...
            // Assert
            Assert.AreEqual(builder.Server, output.Server, StringComparer.Ordinal);
            Assert.AreEqual(builder.Port, output.Port);
            Assert.AreEqual(builder.Database, output.Database, StringComparer.Ordinal);
            Assert.AreEqual(builder.UserId, output.UserId, StringComparer.Ordinal);
        ...
        [TestMethod]
        public void TestAuroraDbConnectionStringBuilderPortForNonNumericValueFallsBackToDefault()
        {
            // Act
            var builder = new AuroraDbConnectionStringBuilder("Server=localhost;Port=not-a-number;");
